...
<div wire:key="checkout-assets-selector-{{ ($isEdit ?? false) ? 'edit-' . ($batchId ?? 0) : 'create' }}-{{ $warehouseId ?? 'all' }}-{{ $productLineId ?? 'all' }}-{{ $requiredArea ?? 0 }}"
    x-data="{
    ...
    if (this.selectedArea < this.requiredArea) { this.insufficientStockWarning=true; if (this.selectedList.length===0) {
        ...
        if (area <= 0) return; while (true) { let totalLoadedArea=0; let accumulated=0; const suggestedIds=[]; for
            ...
            <template x-if="insufficientStockWarning">
                <div
                    class="p-3.5 bg-danger-50 dark:bg-danger-950/50 border border-danger-300 dark:border-danger-700/80 rounded-xl flex items-start gap-3 text-danger-700 dark:text-danger-300 shadow-sm animate-pulse">
                    <svg class="w-5 h-5 flex-shrink-0 text-danger-600 dark:text-danger-400 mt-0.5" fill="none"
                    ...
                    <div class="text-xs">
                        ...
                        <p class="text-danger-600/90 dark:text-danger-400/90 mt-1">Vui lòng kiểm tra lại số lượng hoặc
                            nhập thêm tài sản vào kho trước khi xuất đợt này.</p>
                    </div>
                </div>
            </template>
            ...
            <div class="flex items-center justify-between gap-3 flex-wrap">
                <div>
                    <label class="text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-1.5">
                        ...
                        <span x-show="isEdit && !addMoreMode">Danh sách thiết bị trong đợt xuất (<span
                                x-text="allAssets.length"></span>)</span>
                        <span x-show="isEdit && addMoreMode" class="text-primary-600 font-semibold">Chọn thêm thiết bị
                            từ kho</span>
                    </label>
                    <p x-show="isEdit && !addMoreMode" class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                        Đã gán xuất: <strong class="text-primary-600 font-semibold" x-text="allAssets.length"></strong>
                        thiết bị
                        <template x-if="selectedArea > 0">
                            <span> · Tổng diện tích: <strong class="text-primary-600 font-semibold"
                                    x-text="selectedArea.toFixed(2)"></strong> m²</span>
                        </template>
                    </p>
                    <p x-show="!isEdit" class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                        ...
                        <span x-show="totalWarehouse > 0"> / Tổng <span x-text="totalWarehouse"></span> thiết bị sẵn
                            sàng</span>
                        <template x-if="selectedArea > 0">
                            <span> · Tổng: <strong class="text-primary-600 font-semibold"
                                    x-text="selectedArea.toFixed(2)"></strong> m²</span>
                        </template>
                    </p>
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <template x-if="isEdit && !addMoreMode">
                        <button type="button" @click="startAddMore()"
                    ...
                    <template x-if="isEdit && addMoreMode">
                        <button type="button" @click="backToExisting()"
                    ...
                    <template x-if="!isEdit && requiredArea > 0">
                        <button type="button" @click="autoSuggestByArea(requiredArea)"
            ...
            <!-- Table Container -->
            <div
                class="border border-gray-200 dark:border-gray-700/80 rounded-xl overflow-hidden bg-white dark:bg-gray-900 shadow-2xs">
                <div @scroll.passive="onScroll($event)"
                    ...
                    <template x-if="isEdit && !addMoreMode">
                        <table class="w-full text-left text-sm border-collapse">
                            <thead
                                class="sticky top-0 bg-gray-50/95 dark:bg-gray-800/95 backdrop-blur-xs text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider z-1 border-b border-gray-200 dark:border-gray-700">
                                <tr>
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                ...
                                <tr x-show="assets.length === 0">
                                    <td colspan="4" class="px-5 py-8 text-center text-gray-400 text-sm">
                                        <span x-show="allAssets.length === 0">Đợt xuất này hiện chưa có thiết bị nào.
                                            Nhấn "+ Thêm từ kho" để chọn thiết bị.</span>
                                        <span x-show="allAssets.length > 0">Không tìm thấy thiết bị nào khớp với từ khóa
                                            tìm kiếm.</span>
                                    </td>
                                </tr>
                            </tbody>
                            ...
                            <thead
                                ...
                                <tr>
                                    <th class="w-10 px-3 py-2.5 text-center">
                                        <input type="checkbox" @click="toggleAllVisible()"
                                    ...
                                    <th class="px-4 py-2.5 font-semibold text-gray-500 dark:text-gray-400">TÊN THIẾT BỊ
                                    </th>
                                    <th class="px-4 py-2.5 font-semibold text-gray-500 dark:text-gray-400">KÍCH THƯỚC
                                    </th>
                                    <th class="px-4 py-2.5 font-semibold text-gray-500 dark:text-gray-400">LOẠI THIẾT BỊ
                                    </th>
                                    <th class="px-4 py-2.5 font-semibold text-gray-500 dark:text-gray-400">TRẠNG THÁI
                                    </th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-800/60">
                                ...
                                <template x-for="item in assets" :key="item.id">
                                    <tr @click="toggle(item)"
                                        ...
                                        <td class="px-3 py-2.5 text-center" @click.stop>
                                            <input type="checkbox" :value="item.id" :checked="isChecked(item.id)"
                                                @change="toggle(item)"
                                                class="rounded border-gray-300 dark:border-gray-600 text-primary-600 focus:ring-primary-500 w-4 h-4 cursor-pointer" />
                                        </td>
                                        <td class="px-4 py-2.5 font-bold text-gray-900 dark:text-white font-mono text-sm tracking-tight"
                                            x-text="item.serial_no"></td>
                                        ...
                                        <td class="px-4 py-2.5">
                                            <span
                                                ...
                                                x-text="item.status_label || 'Sẵn sàng trong kho'"></span>
                                        </td>
                                    </tr>
                                </template>
                                ...
                                <tr x-show="loading && assets.length === 0">
                                    <td colspan="6" class="px-4 py-8 text-center text-gray-400 text-xs">
                                        <div class="inline-flex items-center gap-2">
                                            <svg class="animate-spin w-4 h-4 text-primary-500" fill="none"
                                                viewBox="0 0 24 24">
                                ...
                                <tr x-show="!loading && assets.length === 0">
                                    <td colspan="6" class="px-4 py-8 text-center text-gray-400 text-sm">
                                        Không có thiết bị phù hợp với điều kiện lọc (Kho hàng / Loại thiết bị / Tìm
                                        kiếm).
                                    </td>
                                </tr>
                            </tbody>
                    ...
                    <div x-show="loadingMore"
                        class="py-2.5 text-center bg-gray-50/50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-800 text-xs text-primary-600 dark:text-primary-400 font-medium flex items-center justify-center gap-2">
                    ...
                    <div x-show="!loading && !loadingMore && !hasMore && assets.length > 50 && (!isEdit || addMoreMode)"
